...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type, output_channel:Literal['rgb', 'lang'],lf_path,data_type,args):
    ...
    render_path = os.path.join(model_path, save_name, "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, save_name, "ours_{}".format(iteration), "gt")
    render_npy_path = os.path.join(model_path, save_name, "ours_{}".format(iteration), "renders_npy")
    gts_npy_path = os.path.join(model_path, save_name, "ours_{}".format(iteration), "gt_npy")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background,None, cam_type=cam_type,args=args,stage=args.load_stage)[output_channel_key]
        ...
        if output_channel == "rgb":
            ...
            if ONLY_EVAL== 't':
                ...
                gt, mask  = view.get_language_feature(language_feature_dir=lf_path, feature_level=args.feature_level, split=name, data_type=data_type)
                gt_nonorm_list.append(gt)
    ...
    if not args.nonpy:
        ...
        for idx in tqdm(range(len(gt_nonorm_list)), desc="Saving progress"):
            np.save(os.path.join(render_npy_path, '{0:05d}'.format(idx) + ".npy"),render_list[idx].permute(1,2,0).cpu().numpy())
            if (data_type!="dynerf" or name!="video") and ONLY_EVAL=='f':
                np.save(os.path.join(gts_npy_path, '{0:05d}'.format(idx) + ".npy"),gt_nonorm_list[idx].permute(1,2,0).cpu().numpy())
    else:
    ...
    if not args.novideo:
        print("Saving video")
        imageio.mimwrite(os.path.join(model_path, save_name, "ours_{}".format(iteration), f'video_{output_channel}.mp4'), render_images, fps=30)
        if (data_type!="dynerf" and name=="video") and ONLY_EVAL=='f':
            imageio.mimwrite(os.path.join(model_path, save_name, "ours_{}".format(iteration), f'video_gt_{output_channel}.mp4'), gt_images, fps=30)
    else:
        print("For speed up, don't render video")
    
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, mode:Literal['rgb', 'lang'],args):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, hyperparam)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, load_stage=args.load_stage)
        cam_type=scene.dataset_type
        bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background,cam_type,mode,dataset.lf_path,scene.dataset_type, args)
        if not skip_video:
            render_set(dataset.model_path, "video", scene.loaded_iter,scene.getVideoCameras(),gaussians,pipeline,background,cam_type,mode,dataset.lf_path,scene.dataset_type,args)

        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background,cam_type,mode,dataset.lf_path,scene.dataset_type,args)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), hyperparam.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_video,args.mode,args)