...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
                         ...
                         gaussians, scene, stage:Literal['coarse-base','coarse-lang','fine-base','fine-lang','fine-lang-discrete'],joint_train, tb_writer, train_iter,args,timer):
    first_iter = 0

    if joint_train == True:
        assert 'lang' in stage
    logger.info(f"stage:{stage} begin... train_iter:{train_iter}, joint_train:{joint_train}")
    if 'discrete' in stage:
        
        (model_params, first_iter) = torch.load(checkpoint, weights_only=False)
        gaussians.restore(model_params, opt,stage=stage,joint_train=joint_train,no_dlang=args.no_dlang,init_from_stage=args.init_from_stage)
    else:
    ...
    if opt.dataloader:
        ...
        if opt.custom_sampler is not None:
            ...
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size,sampler=sampler,num_workers=0,collate_fn=list, worker_init_fn=seed_worker)
            random_loader = False
        else:
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size,shuffle=True,num_workers=0,collate_fn=list, worker_init_fn=seed_worker)
            random_loader = True
    ...
    for iteration in range(first_iter, final_iter+1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    ...
                    net_image = render(custom_cam, gaussians, pipe, background, scaling_modifer, stage=stage, cam_type=scene.dataset_type,args=args)["render"]
        ...
        if 'base' in stage:
            ...
            Ll1 = args.lam*l1_loss(language_feature_tensor*language_feature_mask_tensor, gt_language_feature_tensor*language_feature_mask_tensor)
            resdict['lang_l1'] = Ll1.item()
            if os.getenv("addcosloss",'f') == 't':
                cosloss = cos_loss(language_feature_tensor*language_feature_mask_tensor, gt_language_feature_tensor*language_feature_mask_tensor)
        ...
        if iteration % log_iter_interval == 0:        
            ...
            # 
            concatenated_image.save(os.path.join(save_path, f'output_{stage}_{iteration}.png'))
            if os.getenv('wandb','f') == 't':
        ...
        if loss.item()>0.5 and iteration > 100 and 'coarse-lang' not in stage:
            
            logger.info("loss",loss)
            logger.info("loss bigger than 0.5,end training, reexecv program now.")
        ...
        with torch.no_grad():
            ...
            # Log and save
            timer.pause()
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, [pipe, background, opt], stage, scene.dataset_type,args)
            ...
            if dataset.render_process:
                if (iteration < 1000 and iteration % 10 == 9) \
                    or (iteration < 3000 and iteration % 50 == 49) \
                        ...
                        render_training_image(scene, gaussians, [test_cams[iteration%len(test_cams)]], render, pipe, background,opt, stage+"test", iteration,timer.get_elapsed_time(),scene.dataset_type,args)
                        render_training_image(scene, gaussians, [train_cams[iteration%len(train_cams)]], render, pipe, background,opt, stage+"train", iteration,timer.get_elapsed_time(),scene.dataset_type,args)
            ...
            if (iteration in checkpoint_iterations) and "fine" in  stage:
                ...
                torch.save((gaussians.capture(opt.include_feature), iteration), scene.model_path + "/chkpnt" +f"_{stage}_" + str(iteration) + ".pth")
    logger.info(f"total_time_ongt:{total_time_ongt}")
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname,timestamp,args):
    # first_iter = 0
    opt.iterations = opt.coarse_base_iterations + opt.coarse_lang_iterations + opt.fine_base_iterations + opt.fine_lang_iterations
    ...
    if args.resume_from_final_stage == 1:
        timer.start()
        scene = Scene(dataset, gaussians, load_iteration=args.resume_from_final_stage_load_iter,load_stage=args.init_from_stage)
        ...
        if opt.coarse_base_iterations>0:
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                    checkpoint_iterations, checkpoint, debug_from,
                                    gaussians, scene, "coarse-base", False, tb_writer, opt.coarse_base_iterations,args,timer)
        if opt.coarse_lang_iterations>0:
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                    checkpoint_iterations, checkpoint, debug_from,
                                    gaussians, scene, "coarse-lang", args.joint_coarse, tb_writer, opt.coarse_lang_iterations,args,timer)
        if opt.fine_base_iterations>0:
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                checkpoint_iterations, checkpoint, debug_from,
                                gaussians, scene, "fine-base", False, tb_writer, opt.fine_base_iterations,args,timer)
        if opt.fine_lang_iterations>0:
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                checkpoint_iterations, checkpoint, debug_from,
                                gaussians, scene, "fine-lang", args.joint_fine, tb_writer, opt.fine_lang_iterations,args,timer)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, stage, dataset_type,args):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians,stage=stage, cam_type=dataset_type,args=args, *renderArgs)["render"], 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000,10000,20000])
    ...
    logger.add(os.path.join(base_save_path,"log",log_file_name), rotation="500 MB")  # 将日志写入文件，当文件大小达到500MB时进行轮转
    logger.info(args)
    ...
    training(lp.extract(args), hp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.expname, timestamp,args)
