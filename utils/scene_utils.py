...
import copy
@torch.no_grad()
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background,opt, stage, iteration, time_now, dataset_type,args):
    def render(gaussians, viewpoint, path, scaling, cam_type,args):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(viewpoint, gaussians, pipe, background,opt, stage=stage, cam_type=cam_type,args=args)
    ...
    for idx in range(len(viewpoints)):
        ...
        render(gaussians,viewpoints[idx],image_save_path,scaling = 1,cam_type=dataset_type,args=args)
